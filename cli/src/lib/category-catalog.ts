/**
 * Static catalog of oxlint categories → rule names (lint-ignore SPEC §3.1.1,
 * PLAN T3.1).
 *
 * Why static: the slash command `/lint:ignore:add` and `commands/ignore/add.ts`
 * need to resolve `category:correctness` (and similar) to the concrete set of
 * rules they will silence in an override block — that resolution must be
 * deterministic, offline, and not depend on spawning `oxlint --rules` on every
 * invocation. The trade-off is that this file drifts from upstream as oxlint
 * adds rules; the smoke test in `cli/tests/unit/category-catalog.test.ts`
 * compares the installed `oxlint` major version against `OXLINT_PINNED_MAJOR`
 * below and fails loudly when they diverge, signalling a quarterly review.
 *
 * To refresh: run `node_modules/.bin/oxlint --rules` and rebuild this file
 * (the rule list per category lives in the `## Correctness (N)` blocks of that
 * output). Keep entries sorted alphabetically per category — `getCategoryRules`
 * returns the array as-is, so consumers can rely on stable iteration.
 *
 * Upstream reference: https://oxc.rs/docs/guide/usage/linter/rules.html
 */

/** oxlint major version this catalog was generated against. Bump together
 * with the rule lists below; the smoke test enforces the contract. */
export const OXLINT_PINNED_MAJOR = 1;
...
const CATEGORY_RULES: Readonly<Record<Category, readonly string[]>> = Object.freeze({
  correctness: [
...
export function isKnownCategory(name: string): name is Category {
  return (KNOWN_CATEGORIES as readonly string[]).includes(name);
}

/** Returns the (stable, sorted) list of rules belonging to `category`.
 *  Caller must not mutate the returned array — it's the same instance held
 *  inside the frozen catalog. */
export function getCategoryRules(category: Category): readonly string[] {
  return CATEGORY_RULES[category];
}

export function getCategorySize(category: Category): number {
  return CATEGORY_RULES[category].length;
}
