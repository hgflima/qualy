...
O preâmbulo `QUALY_BIN=…` está definido em `skills/lint/SKILL.md` (Resolução do CLI). Reuse-o em cada chamada Bash. Stack envelope: TS/TSX/JS/JSX.
...
- Smoke: `node "$QUALY_BIN" ignore-add --help` retorna usage com `<glob>` REQUIRED + `--reason` REQUIRED + `--rule`/`--expires`/`--strict`/`--i-know-this-disables-many`/`--cwd` opcionais.
