...
8. **Aplicação:** `ignore-add <glob> [--rule <id> [--i-know-this-disables-many]] --reason "<reason>" [--expires <date>] --strict --cwd "$PWD"`. Exit `0` aplicado, `1` recoverable (`invalid_glob`, `reason_required`, `expires_in_past`, `unknown_rule`, `unknown_category`, `category_requires_ack`, `decisions_failed`), `3` dirty tree (volte ao passo 2), `4` usage, `70` `manifest_corrupt`.
9. **Pós-condição:** mostre `id` do entry, `action` (`added` / `updated`), `imported.length` (patterns brownfield-importados nessa mutação), `files_changed` (manifesto + presets + decisions). NÃO commite — SPEC §6 Never. Sugira `/lint:audit` para re-medir e `/lint:ignore:list` para visualizar.
