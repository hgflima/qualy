...
- Smoke: `node "$QUALY_BIN" audit --cwd "$PWD" --tier fast` num fixture com `oxlint.fast.json` retorna exit `0` e grava `.lint-audit/<ts>.json` válido contra `auditPayloadSchema`.
